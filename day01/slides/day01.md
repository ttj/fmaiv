...
> "We're now at the stage where I will get suggestions from collaborators I've never met, mediated by a machine, on a proof I've never written, in a language I've barely learned, and the kernel arbitrates whether they're right." — Terence Tao, on collaborative Lean formalization, 2024

::: notes
This is the framing slide for the whole week. The story is: AI is doing the easy half of verification (proposing) much faster than humans can do the hard half (checking). Formal methods is the only known way to mechanize the checking. We're not here to compete with AI; we're here to build the substrate it needs to be trustworthy. The Tao quote is real and from a 2024 blog post — I'll cite it on the references slide.
:::
...
- **Polynomial Freiman–Ruzsa conjecture** — formalized in Lean by Tao, Gowers, Green, Manners and a public collaboration of ~25 people. Three weeks from informal proof to checked formalization (October 2023).
...
- **29–45% of LLM-generated code carries known security vulnerabilities** (multiple 2024–25 audits — CodeXGLUE, BigVul, OWASP studies).
...
- **AWS s2n-TLS, by contrast, is continuously formally verified** — every commit triggers SAW proofs of crypto correctness; AES-XTS support was formally proven in March 2026.
...
**Therac-25** (1985–87) — radiation therapy machine, six deaths.
...
**Ariane 5, Flight 501** (1996) — €370M rocket, $500M payload, exploded 37 seconds after launch.
...
**Toyota unintended acceleration** (2007–10) — multiple fatal accidents, $1.2B settlement.
...
> "We treat verification as a CI signal, like tests. The kernel says yes or it says no." — Byron Cook, AWS Distinguished Scientist (2024)

::: notes
AWS is the showcase deployment. Byron Cook's group at AWS Provable Security is the largest industrial formal methods group in the world, and they publish almost everything. If you want to read industrial-strength SAW scripts, the s2n-bignum repo is the place. The Cedar work is particularly relevant for Day 3 — it's Lean used as a *specification* language for production-deployed software, not just a math library.
:::
